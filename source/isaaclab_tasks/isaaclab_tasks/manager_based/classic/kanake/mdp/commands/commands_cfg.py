...
from isaaclab.markers.config import BLUE_ARROW_X_MARKER_CFG, SPHERE_MARKER_CFG, POSITION_GOAL_MARKER_CFG, CUBOID_MARKER_CFG, FRAME_MARKER_CFG, GREEN_ARROW_X_MARKER_CFG
...
from .kanake_base_command import KanakeBaseCommand
from .KanakeUniformVelocityCommand import KanakeUniformVelocityCommand
...
@configclass
class KanakeWorldCommandCfg(CommandTermCfg):
    """Configuration for the uniform z/heading command generator."""

    class_type: type = KanakeWorldCommand
    ...
    @configclass
    class Ranges:
        """Ranges for sampling commands."""
        # 목표 Z 위치의 샘플링 범위 (미터)
        pos_z: tuple[float, float] = MISSING
        # 목표 Pitch의 샘플링 범위 (라디안)
        pitch: tuple[float, float] = MISSING
...
@configclass
class KanakeBaseCommandCfg(CommandTermCfg):
    """Configuration for the uniform 2D-pose command generator."""

    class_type: type = KanakeBaseCommand
    ...
    goal_pose_visualizer_cfg.markers["cuboid"].size = (0.1, 0.1, 0.1)
    goal_pose_visualizer_cfg.markers["cuboid"].visual_material = sim_utils.PreviewSurfaceCfg(diffuse_color=(0.0, 1.0, 0.0))


@configclass
class KanakeUniformVelocityCommandCfg(CommandTermCfg):
    """Configuration for the uniform velocity command generator."""

    class_type: type = KanakeUniformVelocityCommand
