...
class JointSineAction(ActionTerm):
    ...
    def process_actions(self, actions: torch.Tensor, additional_joint_values: torch.Tensor = None):
        ...
        # 주파수 : 0.3 ~ 1.1
        freq_v = 0.3 + 0.8 * (torch.tanh(freq_v_action) + 1.0) / 2.0
        freq_h = 0.3 + 0.8 * (torch.tanh(freq_h_action) + 1.0) / 2.0
        # freq_v = 1.1 * (torch.tanh(freq_v_action)) 
        # freq_h = 1.1 * (torch.tanh(freq_h_action)) 

        # 위상 (Phase)
        phase_v = np.pi/4 * torch.tanh(phase_v_action+ 1.0) / 2.0 
        phase_h = np.pi/4 * torch.tanh(phase_h_action+ 1.0) / 2.0
        # # 진폭 (Amplitude)
