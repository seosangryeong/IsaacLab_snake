...
class JointSineAction(ActionTerm):
    """
    이 액션 term은 각 환경마다 (조인트 개수 + 2)개의 파라미터를 사용
      - 각 조인트별 진폭 (num_joints)
      - 수직 조인트: phase_vertical (양수/음수 선택)
      - 수평 조인트: phase_horizontal (양수/음수 선택)
    
    주파수는 고정값 사용 (freq_v = 1.0, freq_h = 1.0)
    위상은 양수면 +π/4, 음수면 -π/4
    ...
    def process_actions(self, actions: torch.Tensor, additional_joint_values: torch.Tensor = None):
        ...
        phase_v_action = actions[:, self._num_joints]      # 수직 위상 선택
        phase_h_action = actions[:, self._num_joints + 1]   # 수평 위상 선택
        ...
        freq_h = torch.full((self.num_envs,), self._fixed_freq_h, device=self.device)

        # 위상: 양수면 +π/4, 음수면 -π/4 (이진 선택)
