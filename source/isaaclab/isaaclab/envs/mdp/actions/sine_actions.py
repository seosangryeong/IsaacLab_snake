...
class JointSineAction(ActionTerm):
    ...
    def process_actions(self, actions: torch.Tensor, additional_joint_values: torch.Tensor = None):
        ...
        # 진폭 (Amplitude)
        amplitudes = 1.0 + 0.3 *torch.tanh(amp_actions)

        # 주파수 (Frequency)
        freq_v = 1.0 + 0.3 * (torch.tanh(freq_v_action) + 1.0) / 2.0
        freq_h = 0.5 + 0.3 * (torch.tanh(freq_h_action) + 1.0) / 2.0

        # 위상 (Phase)
        phase_v = 2.0 + 0.3 * torch.tanh(phase_v_action)
        phase_h = 1.0 + 0.2 * torch.tanh(phase_h_action)
