...
class JointSineHorizonAction(ActionTerm):
    """사인파 기반 액션 term.  
    수직 조인트는 사용하지 않고 수평 조인트에만 사인파를 적용하는 액션
    
      - 수평 조인트 (조인트 이름: j2, j4, j6, ...): amplitude, frequency, phase  
    
       position = amplitude * sin(2π * frequency * t + (조인트 번호) * phase)
    """
    
    cfg: actions_cfg.JointSineHorizonActionCfg
    _asset: Articulation
    _current_time: float

    def __init__(self, cfg: actions_cfg.JointSineHorizonActionCfg, env: ManagerBasedEnv) -> None:
        ...
        if isinstance(self._joint_ids, list):
            ...
            self._horizontal_joint_ids = list(range(self._num_joints))[1::2]  # 짝수 인덱스만
    ...
    def process_actions(self, actions: torch.Tensor):
        ...
        # 수평 조인트 위치 계산
        horizontal_pos = amplitude.unsqueeze(1) * torch.sin(
            2 * np.pi * frequency.unsqueeze(1) * t + horizontal_numbers * phase.unsqueeze(1)
        )
    ...
    def apply_actions(self):
        """계산된 관절 위치를 수평 조인트의 목표 위치로 설정"""
        self._asset.set_joint_position_target(self._processed_actions, joint_ids=self._horizontal_joint_ids)
